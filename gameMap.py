...
class GameMap():
    ...
    def create_new_map(self, room_id:str, player_ids:list):
        ...
        new_game_map = {
            ...
            "players_in_map_info": {player_id: {"node_code": 0, "node_name": "会议室"} for player_id in player_ids},
            "players_in_map_info_history": {player_id: [{"node_code": 0, "node_name": "会议室"}] for player_id in player_ids}
        }
    ...
    def get_node(self, node_code_or_name: str):
        for node in MAP_NODES:
            if node["NodeCode"] == node_code_or_name or node["NodeName"] == node_code_or_name:
    ...
    def get_next_nodes(self, player_id:str, node_code_or_name: str, in_vent: bool = False) -> list:
    ...
    def move_to_next_node(self, player_id: str, next_node_code: int, in_vent: bool = False):
        room_id = spm.get_player_room_unique()[player_id]
        map_id, game_map = self.get_map_from_room(room_id)
        if map_id is None or game_map is None:
            raise ValueError(f"玩家{player_id}不在任何房间中,查询位置失败")
        if not in_vent:
            # 检查移动是否合法
            if next_node_code not in self.get_next_nodes(player_id, game_map["players_in_map_info"][player_id]["node_code"]):
                raise ValueError(f"玩家{player_id}无法移动到节点{next_node_code}")
            for node in game_map["map_nodes"]:
                if node["NodeCode"] == next_node_code:
                    game_map["players_in_map_info"][player_id]["node_code"] = next_node_code
                    game_map["players_in_map_info"][player_id]["node_name"] = node["NodeName"]
                    game_map["players_in_map_info_history"][player_id].append({"node_code": next_node_code, "node_name": node["NodeName"]})
    ...
    def get_player_node_history(self, player_id: int):
        ...
        if map_id is None or game_map is None:
            raise ValueError(f"玩家{player_id}不在任何房间中,查询位置失败")
        return "→".join([node["NodeName"] for node in game_map["players_in_map_info_history"][player_id]])
    ...
    def force_move_player_to_node(self, player_id: str, target_node_code: int):
        ...
        for node in game_map["map_nodes"]:
            if node["NodeCode"] == target_node_code:
                game_map["players_in_map_info"][player_id]["node_code"] = target_node_code
                game_map["players_in_map_info"][player_id]["node_name"] = node["NodeName"]
                game_map["players_in_map_info_history"][player_id].append({"node_code": target_node_code, "node_name": node["NodeName"]})
                return True
        return False